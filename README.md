...
Теги: Python, SQLAlchemy, PostgreSQL, Alembic, Pydantic, Async, Aiogram, Hugging Face

## Описание:
- Телеграм-бот с помощью Aiogram, принимает текст от пользователя и отвечает на заданные вопросы о количестве.
- LLM принимает сообщение от пользователя и переводит в SQL запрос, используется Hugging Face.
- Для работы с БД используется SQLAlchemy и для миграций Alembic.
- Для загрузки окружения используется Pydantic.
- Созданы функции записи из JSON в БД.
